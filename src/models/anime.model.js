...
import { createDataFile, getAllDataAnime, updateAnime, deleteAnime, getAnimeById, getAnimeByName } from '../utils/fileUtils.js';
...
export class Anime {
    ...
    static async getName(nombre) {
        try {
            const anime = await getAnimeByName(nombre, 'animes.json')
            return anime
        } catch (error) {
            throw new Error("Error getting anime data", error);
        }
